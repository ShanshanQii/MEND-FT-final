PROGRAM MEND_main
    ...
    real(8) fObj !!,fObj_cr(2)  !!upper and lower bound for COFI
    ...
    REAL(8), ALLOCATABLE:: dSIM_t01d(:)
    REAL(8), ALLOCATABLE:: dSIM_t0(:,:)
    REAL(8), ALLOCATABLE:: dSIM_t(:,:)
    REAL(8), ALLOCATABLE:: Sobol_obj(:), Sobol_obj_slave(:)

    INTEGER nSA, nSA0 !! number of parameter sets for Sobol Sensitivity Analysis
    REAL(8) rRead
    REAL(8), ALLOCATABLE:: xpar(:,:), xpar0(:,:)
    REAL(8), ALLOCATABLE:: xpar1d(:)
    REAL(8), ALLOCATABLE:: yvar(:)
    ...
    if(pid == 0) then
        ...
        print*, "*--------Carbon-Nitrogen Coupled MEND---------*"
        print*, "*------------MPI Parallel Version-------------*"
        print*, "*----wang.gangsheng@gmail.com; Oct 10, 2021---*"
        print*, "*---------------------------------------------*"
