...
CONTAINS
    ...
    SUBROUTINE SCE(sSCE, sPAR, sINI, sOUT)
        ...
        REAL(8) denomi, timeou, gnrng, pcento
      
        INTEGER iOpt(sSCE%nOpt)
        REAL(8) a(sSCE%nPar),bl(sSCE%nPar),bu(sSCE%nPar)
        REAL(8) xx(sSCE%nPar),bestx(sSCE%nPar),worstx(sSCE%nPar)
        REAL(8) xnstd(sSCE%nPar),bound(sSCE%nPar),criter(20),unit(sSCE%nPar)
      
        REAL(8) x(sSCE%npt,sSCE%nPar),xf(sSCE%npt)
        REAL(8) cx(sSCE%npg,sSCE%nPar),cf(sSCE%npg)
        REAL(8) s(sSCE%nps,sSCE%nPar),sf(sSCE%nps)
...
820  format(//,1x,'*** OPTIMIZATION TERMINATED BECAUSE THE POPULATION',&
 ...
 SUBROUTINE cce(sSCE, sPAR, sINI, sOUT, s,sf,xnstd,iCALL)
     ...
     INTEGER iCALL !!, iseed
     !      REAL(8):: s(50,sSCE%nPar),sf(50),xnstd(sSCE%nPar)
     REAL(8):: s(:,:),sf(:),xnstd(:)
      
     !!LOCAL VARIABLES:
     INTEGER :: iOpt(sSCE%nOpt)
     REAL(8) :: bu(sSCE%nPar),bl(sSCE%nPar)
     REAL(8) sw(sSCE%nPar),sb(sSCE%nPar),ce(sSCE%nPar),snew(sSCE%nPar)
