...
CONTAINS
    ...
    END !!FUNCTION
    !------------------------------------------------------------------
    REAL(8) function fRang(nArray, Dataobs, Datasim, tolerance, dFill)     !2023/8/12 added
        INTEGER nArray
        REAL(8), DIMENSION(nArray) :: Dataobs, Datasim
        REAL(8) :: tolerance, dFill
        ...
        do i = 1, m
            j = iData(i)
            if (DABS(Datasim(j) - Dataobs(j))/max(DABS(Dataobs(j)), dLOW).gt. tolerance) then
                sum1=sum1+1
            end if
        end do

        if(sum1/REAL(m) .gt. 0.5) then    !2023/8/12 added
          fRang = 999999
        else 
          fRang = 0.d0
        end if
    ...
    !------------------------------------------------------------------
    REAL(8) function fMSE(nArray, Dataobs, Datasim, ifNorm, dFill)
    ...
    SUBROUTINE Array_2dto1d(nrow, ncol, array2d, array1d)
        ...
        REAL(8), intent(in) :: array2d(nrow, ncol)
        REAL(8), intent(out):: array1d(nrow*ncol)
    ...
    SUBROUTINE Array_1dto2d(array1d, nrow, ncol, array2d)
        ...
        !!ARGUMENTS:
        INTEGER, intent(in) :: nrow, ncol
        REAL(8), intent(out) :: array2d(nrow, ncol)
        REAL(8), intent(in):: array1d(nrow*ncol)
